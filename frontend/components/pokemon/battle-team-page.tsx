...
export function BattleTeamPage() {
  ...
  const addRandomToTeam = () => {
    if (battleTeam.length >= maxTeamSize || availablePokemon.length === 0)
      return;
  ...
  const getTeamStats = () => {
    if (battleTeam.length === 0)
      return { avgHp: 0, avgAttack: 0, avgDefense: 0, totalPower: 0 };
    ...
    return {
      ...
      totalPower: Math.round(
        (stats.hp + stats.attack + stats.defense) / battleTeam.length
      ),
    };
  };
          ...
          {battleTeam.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-4">
                Adicione até {maxTeamSize} Pokémon à sua equipe de batalha
              </p>
              <Button
                variant="outline"
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                ...
                {/* Empty Slots */}
                {Array.from({ length: maxTeamSize - battleTeam.length }).map(
                  (_, index) => (
                    <Card
                      key={`empty-${index}`}
                    ...
                    >
                      <CardContent className="p-4 h-full flex flex-col items-center justify-center text-muted-foreground">
                        ...
                        <span className="text-xs text-center">
                          Slot {battleTeam.length + index + 1}
                        </span>
                      </CardContent>
                    </Card>
                  )
      ...
      {showAvailable && (
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle>
                Pokémon Disponíveis ({availablePokemon.length})
              </CardTitle>
              <Button
                variant="outline"
                size="sm"
