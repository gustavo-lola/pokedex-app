...
export function PokemonCard({ pokemon, userPokemon, showActions = true }: PokemonCardProps) {
  const { toggleFavorite, addToBattleTeam, removeFromBattleTeam, battleTeam } = usePokemon()
              ...
              {showActions && (
                <div className="flex gap-1">
                  ...
                  <Button
                    ...
                    className={`h-6 w-6 p-0 ${isInBattleTeam ? "text-yellow-500" : "text-muted-foreground"}`}
                  >
                    {isInBattleTeam ? <Star className="h-3 w-3 fill-current" /> : <Plus className="h-3 w-3" />}
                  </Button>
                </div>
              )}
            </div>

            <h3 className="font-semibold text-lg capitalize text-balance">{pokemon.name}</h3>
            ...
            <div className="flex gap-1 justify-center flex-wrap">
              {pokemon.types.map((type) => (
                <Badge
                  ...
                  style={{ backgroundColor: PokemonAPI.getTypeColor(type.type.name) }}
                >
                  {type.type.name}
                </Badge>
              ))}
                  ...
                  <div key={statName} className="space-y-1">
                    ...
                    <div className="stat-bar h-2">
                      <div
                        ...
                        style={{
                          width: `${percentage}%`,
                          background: `linear-gradient(to right, ${typeColor}, ${PokemonAPI.getTypeColor(pokemon.types[1]?.type.name || primaryType)})`,
                        }}
                      />
                    </div>
                  </div>
