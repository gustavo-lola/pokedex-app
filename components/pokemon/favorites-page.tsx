...
export function FavoritesPage() {
  ...
  const filteredFavorites = favorites
    .filter((userPokemon) => {
      const matchesType =
        selectedType === "all" || userPokemon.pokemon.types.some((type) => type.type.name === selectedType)
      return matchesType
    })
    .sort((a, b) => {
      switch (sortBy) {
  ...
  const availableTypes = Array.from(
    new Set(favorites.flatMap((userPokemon) => userPokemon.pokemon.types.map((type) => type.type.name))),
  ).sort()
          ...
          {favorites.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-4">
                Adicione Pokémon aos seus favoritos clicando no coração nos cards
              </p>
            ...
            <div className="space-y-4">
              ...
              {filteredFavorites.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground">Nenhum favorito encontrado com os filtros aplicados</p>
                </div>
              ) : (
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center">
                ...
                <div className="text-sm text-muted-foreground">Total de Favoritos</div>
              </div>
              <div className="text-center">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-accent">
                  {Math.round(
                    favorites.reduce(
                      (sum, up) => sum + up.pokemon.stats.find((s) => s.stat.name === "hp")!.base_stat,
                      0,
                    ) / favorites.length,
                  )}
                </div>
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-chart-1">
                  {Math.round(
                    favorites.reduce(
                      (sum, up) => sum + up.pokemon.stats.find((s) => s.stat.name === "attack")!.base_stat,
                      0,
                    ) / favorites.length,
                  )}
                </div>
