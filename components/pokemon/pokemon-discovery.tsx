          ...
          {searchResults.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {searchResults.map((pokemon) => (
                <PokemonCard key={pokemon.id} pokemon={pokemon} userPokemon={getUserPokemon(pokemon.id)} />
              ))}
            </div>
          )}
          ...
          {randomPokemon && (
            <div className="flex justify-center">
              <div className="w-full max-w-xs">
                <PokemonCard pokemon={randomPokemon} userPokemon={getUserPokemon(randomPokemon.id)} />
              </div>
            </div>
          )}
        </CardContent>
        ...
        <CardContent>
          {filteredDiscovered.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm">Use a busca ou descoberta aleatória para começar!</p>
            </div>
          ) : (
